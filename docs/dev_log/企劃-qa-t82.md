...
### [2026-06-08] — Bet→Scatter 機率可行性分析

**環節：** 數學
**AI 工具：** Kiro Agent
**做了什麼：** 分析「Bet 越大 → Scatter 出現機率越高 → 更快觸發 FG/BG」的可行性。跑多組模擬（SC 率 2%~8%、lamp 加速、門檻調整），發現無法在所有 bet 等級同時維持 RTP 95.5%~96.5%。推薦方案 A（純視覺加速感）。
**Before：** 用戶提出 Bet→Scatter 動態機率需求
**After：** 確認此需求在當前賠付結構下不可行，提出 3 種替代方案
